...
using Newtonsoft.Json.Linq; // Asegúrate de que JTokenType esté disponible
...
_logger.LogDebug("✅ Mensaje procesado y confirmado");
                ...
                {
                    _logger.LogError(ex, $"❌ Error procesando mensaje: {ex.Message}");
...
var email = json["email"]?.ToString();
            ...
            // Asumimos que el typo 'userNaame' en los logs es un error de copiado
            // y que la clave real es 'userName' como está en el código.
...
return;
            ...
            // 1. Obtener el token 'additionalData' de forma segura
            var additionalDataToken = json["additionalData"];
            
            // 2. Verificar si NO es nulo ANTES de intentar usarlo
            Dictionary<string, object>? additionalData = null;
            if (additionalDataToken != null && additionalDataToken.Type != JTokenType.Null)
            {
                additionalData = additionalDataToken.ToObject<Dictionary<string, object>>();
            }
            ...
            {
                case "user_welcome":
                    // 3. Usar el diccionario 'additionalData' de forma segura
                    var activationToken = additionalData?.GetValueOrDefault("activationToken")?.ToString();
                    await emailService.SendWelcomeEmailAsync(email, userName, activationToken);
                break;

                case "login_notification":
                    // 4. Reutilizar el diccionario 'additionalData'
                    await emailService.SendLoginNotificationAsync(email, userName, additionalData);
                break;

                case "password_reset":
                    // 5. Usar el diccionario 'additionalData' de forma segura
                    var resetToken = additionalData?.GetValueOrDefault("resetToken")?.ToString() ?? "";
